...
func init() {
	...
	pflags := rootCmd.PersistentFlags()
	pflags.StringVar(&rootConfigFile, "rootConfig", "", "root config file, could be overrided (default is $GOPATH/src/api/config/config.yaml)")
...
func initRootConfig() {
	if rootConfigFile != "" {
		...
		for _, goPath := range goPaths {
			configDir := filepath.Join(goPath, "src", "api", "config")
